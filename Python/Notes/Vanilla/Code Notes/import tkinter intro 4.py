...
class MyGUI():
    def __init__(self):
        ...
        self.textbox.bind("<KeyPress>", self.shortcut) # THIS WILL BIND FUNCTIONS
        # <KeyPress> is a must name
        ...
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing) # CALL WHEN BUTTON CLOSE IS CLICKED
        self.root.mainloop()
    ...
    def shortcut(self, event): # EVENT IS WHAT HAPPEND WHEN KEYBOARD PRESSED
        if event.state == 12 and event.keysm == "Return":
            self.show_message()
        
    def on_closing(self): # THIS WELL GET CALLED WHEN close BUTTON IS CLICKED
